...
class UdacitySelfDrivingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, transform, train=True, thinning=None):
        ...
        # Create a mapping from consecutive indices to actual image_ids
        self.valid_indices = sorted(self.df.index.unique())
    ...
    def __getitem__(self, idx):
        ...
        # Map the consecutive index to the actual image_id
        actual_idx = self.valid_indices[idx]
        
        if actual_idx in self.df.index:
            row = self.df.loc[[actual_idx]]
        else:
            raise KeyError(f"Element {actual_idx} not in dataframe")
    ...
    def __len__(self):
        return len(self.valid_indices)  # Return length of valid indices, not n_images
